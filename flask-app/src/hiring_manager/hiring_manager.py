...
@hiring_manager.route('/hiring_manager/<userID>/jobs', methods=['GET'])
def get_hiring_manager_jobs(userID):
    cursor = db.get_db().cursor()
    cursor.execute('select title, description from job where hiring_manager_id = {0}'.format(userID))
    row_headers = [x[0] for x in cursor.description]
    json_data = []
    theData = cursor.fetchall()
    for row in theData:
